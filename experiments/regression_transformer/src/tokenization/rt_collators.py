# Adapted from Regression Transformer's terminator.collators and collator_utils
...
class BaseCollator:
    ...
    def finalize(self, batch: torch.Tensor, val: int = 0) -> torch.Tensor:
        ...
        if batch.size(1) % 2 != 0:
            return torch.cat(
                [batch, torch.ones(batch.size(0), 1).long() * val], axis=1)
        return batch.long()
    ...
    def find_entity_indices(
            self,
            inputs: torch.Tensor,
            entity_to_mask: Optional[int] = None) -> Tuple[List, List]:
        """
        ...
        sep_matches, sep_token_pos = (
            inputs == self.separator_token_idx).nonzero(as_tuple=True)
        unique_sep_matches = torch.unique(sep_matches)
        assert torch.equal(unique_sep_matches, torch.arange(inputs.size(
            0))), f"Found samples without separator {unique_sep_matches}"
        ...
        if entity_to_mask is None:
            ...
            last_ent_pos = [
                int(
                    torch.min(
                        torch.where(s == self.tokenizer.sep_token_id)[0]) - 1)
                for s in inputs
            ]
        ...
        num_entities = int(
            (inputs == self.entity_separator_idx).sum(axis=1).min()) + 1
        if num_entities == 1 and entity_to_mask is not None:
            raise ValueError(
                f"Cannot mask {entity_to_mask}-th entity, only one found")
        ...
        entsep_matches, entsep_token_pos = (
            inputs == self.entity_separator_idx).nonzero(as_tuple=True)
        unique_entsep_matches = torch.unique(entsep_matches)

        assert torch.equal(unique_entsep_matches, torch.arange(
            inputs.size(0))), "Found samples with only a single entity"
        ...
        if entity_to_mask == 0:
            ...
            first_ent_pos = [
                int(entsep_token_pos[torch.where(
                    entsep_matches == s)][entity_to_mask - 1]) + 1
...
class MaskedTextCollator(BaseCollator):
    ...
    def mask_tokens(self,
        ...
        perm_mask = get_permutation_order(inputs, masked_indices,
                                          non_func_mask)
...
class SinglePropertyCollator(BaseCollator):
    ...
    def __init__(
        ...
        if property_token not in tokenizer.vocab_list:
            raise ValueError(
                f"Property token {property_token} not in tokenizer vocab")
        ...
        if num_tokens_to_mask > 0 and isinstance(mask_token_order, list):
            self.tokens_to_mask = torch.Tensor(
                mask_token_order[:num_tokens_to_mask]).long()

        self.separator_token_idx = tokenizer.vocab[
            tokenizer.expression_separator]
    ...
    def mask_tokens(
            ...
            if list(range(inputs.size(0))) != sample_matches.tolist():
                ...
                if search_idx == self.property_token_idx:
                        ...
                        if not self.ignore_errors:
                            ...
                            x = "".join(
                                self.tokenizer.convert_ids_to_tokens(
                                    inputs[dups[0]]))
                            logger.error(
                                f"Multiple occurrences of property {x}")
                    ...
                    for sample_idx in range(inputs.size(0)):
                        ...
                        match_pos[sample_idx] = sample_sep_pos[torch.where(
                            rel_sep_pos > 0)[0][0]]
        ...
        labels = inputs.clone()
        masked_indices = torch.full(labels.shape,
                                    0,
                                    dtype=torch.bool,
                                    device=device)
        target_mapping = torch.zeros(
        ...
        if self.mask_token_order is None:
            ...
            for i in range(masked_indices.shape[0]):
                if self.property_token_idx in inputs[i]:
        ...
        perm_mask = get_permutation_order(labels,
                                          masked_indices,
                                          non_func_mask,
                                          device=device)
...
class PropertyCollator(SinglePropertyCollator):
    ...
    def __init__(
        ...
        assert len(property_tokens) == len(
            num_tokens_to_mask), "Lengths must match"
        assert len(property_tokens) == len(
            mask_token_order), "Lengths must match"
        ...
        collators = []
        for token, num_to_mask, order in zip(property_tokens,
                                             num_tokens_to_mask,
                                             mask_token_order):
            collators.append(
    ...
    def mask_tokens(
        ...
        perm_mask = torch.zeros(*inputs.shape, inputs.shape[-1],
                                device=device).bool()
...
class ConditionalGenerationEvaluationCollator(BaseCollator):
    ...
    def __init__(
        ...
        self.conditioning_range = list(
            map(
                lambda x: property_token + str(x) + tokenizer.
    ...
    def __call__(
        ...
        batch = _torch_collate_batch(examples, self.tokenizer)
        inputs, perm_mask, target_mapping, labels, true_prop = self.mask_tokens(
            batch)
    ...
    def mask_tokens(
        ...
        if len(a) != inputs.size(0) or list(range(
                inputs.size(0))) != a.tolist():
            raise ValueError(
                f"Found samples with multiple or no token counts {inputs}")
        ...
        for i, p in enumerate(primer_start_pos):
            ...
            primer_end_pos.append(
                int(cans[torch.min(torch.where(cans - p > 0)[0])]))
        ...
        perm_mask = get_permutation_order(labels, masked_indices,
                                          non_func_mask)
        ...
        target_mapping = target_mapping.repeat_interleave(self.num_primed,
                                                          dim=0)
        labels = labels.repeat_interleave(self.num_primed, dim=0)
        return primed_inputs.long(
        ), perm_mask, target_mapping, labels, real_property

    def prime_inputs(
            self, inputs: torch.Tensor, property_token_pos: List,
        ...
        primed_inputs = torch.zeros(self.num_primed * inputs.shape[0],
                                    inputs.shape[1])
        ...
        for i in range(inputs.shape[0]):
            ...
            real_property[i] = self.tokenizer.floating_tokens_to_float(
                self.tokenizer.decode(
                    inputs[i, property_token_pos[i] +
                           1:separator_token_pos[i]]).split(" "))
            ...
            for pidx, primer in enumerate(self.conditioning_range):
                ...
                primed_inputs[
                    ...
                    property_token_pos[i] + 1:property_token_pos[i] + 1 +
                    len(keep),
                ] = torch.Tensor(keep)
...
class ConditionalGenerationTrainCollator(BaseCollator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.property_token_idxs = [
            tokenizer.vocab[p] for p in property_tokens
        ]

        if self.do_sample:
            if not property_value_ranges:
                raise TypeError(
                    "property_value_ranges not provided for sampling.")

            self.property_value_ranges = property_value_ranges
            assert self.num_props == len(
                property_value_ranges), "Ranges missing"
            for pvr in property_value_ranges:
                if not (pvr[1] > pvr[0]):
                    raise ValueError(
                        f"Property range should be sorted ({pvr})")
    ...
    def mask_tokens(
        ...
        last_sep_pos = [
            int(max(separator_token_pos[torch.where(separator_matches == s)]))
            + 1 for s in unique_separator_matches
        ]
    ...
    def sample_property(
        ...
        for pidx, (prop_token, prop_token_idx) in enumerate(
            ...
            property_matches, property_token_pos = (
                inputs == prop_token_idx).nonzero(as_tuple=True)
            if len(set(property_matches.tolist())) < inputs.size(0):
                logger.debug(
                    f"Found samples without property token {prop_token}")
            elif len(property_matches.tolist()) > len(
                    set(property_matches.tolist())):
                raise ValueError(
                    f"Multiple occurrences of {prop_token} not supported.")

            # Mask from property til next separator (samples w/o property are not masked)
            for i, (prow, pcol) in enumerate(
                    zip(property_matches, property_token_pos)):
                # First, extract true property

                # Find all separators in current sample, then find the next one
                separator_idxs = separator_token_pos[torch.where(
                    separator_matches == prow)]
                sep_idx = min([x for x in separator_idxs if x > pcol])

                realprop = self.tokenizer.floating_tokens_to_float(
                    self.tokenizer.decode(inputs[prow,
                                                 pcol + 1:sep_idx]).split(" "))
                real_property[prow, pidx] = realprop
                if self.do_sample:
                    # Sample property and fill it in
                    num_digits = sep_idx - (pcol + 1)
                    sampled_prop = sample(
                        realprop)  # ignored in the base class
                    sampled_prop_str = (prop_token +
                                        str(sampled_prop)[:num_digits] +
                                        self.tokenizer.expression_separator)
                    new_tokens = torch.Tensor(
                        self.tokenizer(sampled_prop_str)["input_ids"])
                    new_tokens = new_tokens[2:torch.where(
                        new_tokens == self.separator_token_idx)[0]]
    ...
    def get_sample_weight(self, real_prop: float, sampled_prop: float,
                          prop_range: float) -> float:
        ...
        """
        return 2 * ((-1 *
                     (abs(real_prop - sampled_prop) / prop_range)) + 1) - 1
...
def get_mask(
    ...
    for i in range(labels.size(0)):
        ...
        while cur_len < max_len:
            ...
            span_length = min(
                torch.randint(1, max_span_length + 1, (1, )).item(),
                max_len - cur_len)
            ...
            start_index = max(
                min(
                    cur_len + torch.randint(context_length - span_length + 1,
                                            (1, )).item(),
                    max_len - span_length,
                ),
                cur_len,
...
def get_permutation_order(
    ...
    for i in range(labels.size(0)):
        ...
        perm_mask[i] = (perm_index.reshape(
            (labels.size(1), 1)) <= perm_index.reshape(
...
TRAIN_COLLATORS = {
    ...
    "plm": DataCollatorForPermutationLanguageModeling,
    "vanilla_cg": ConditionalGenerationTrainCollator,
    "conditional_generation": ConditionalGenerationTrainCollator,
