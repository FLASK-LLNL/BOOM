# Adapted from Regression Transformer's terminator/tokenization.py
import re
...
class ExpressionTokenizer:
    ...
    def __init__(self,
        ...
        if language == "SMILES":
            self.text_tokenizer = RegexTokenizer(
                regex_pattern=SMILES_TOKENIZER_PATTERN)
    ...
    def tokenize(self, text: str) -> List[str]:
        ...
        for property_expression in splitted_expression[:-1]:
            tokens.extend(
                self.property_tokenizer.tokenize(property_expression))
...
class ExpressionBertTokenizer(BertTokenizer):
        ...
        **kwargs,
        ...
        Args:
            ...
            pad_even (bool): Boolean indicating whether sequences of odd length should
                be padded to have an even length. Needed for PLM in XLNet. Defaults to
                True.
            language (str): Identifier for the (chemical) language. Should be either
                'SMILES', 'SELFIES' or 'AAS'.
        """
        ...
        #    pad_to_multiple_of=2 if pad_even else None,
    ...
    def add_padding_tokens(self,
                           token_ids: List[int],
                           max_length: int,
                           padding_right: bool = True) -> List[int]:
    ...
    @staticmethod
    def get_sample_label(mlm_label: List[str],
                         mlm_input: List[str]) -> List[str]:
        ...
        return [
            i if el == "[UNK]" else el for el, i in zip(mlm_label, mlm_input)
        ]

    @staticmethod
    def get_sample_prediction(mlm_prediction: List[str],
                              mlm_input: List[str]) -> List[str]:
    ...
    def floating_tokens_to_float(token_ids: List[str]) -> float:
        ...
        try:
            float_string = "".join(
                [token.split("_")[1] for token in token_ids])
    ...
    def aggregate_tokens(self,
                         token_ids: List[str],
                         label_mode: bool,
                         cls_first: bool = True) -> Tuple[str, Dict]:
        ...
        seq = ("".join(token_ids[token_ids.index("|") + 1:edx])
               if "|" in token_ids else "".join(token_ids))
        property_dict = {}
